...
import { Lock, User, Loader2, Info } from "lucide-react";
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-ink p-4">
      ...
      <div className="glass relative z-10 w-full max-w-md rounded-3xl p-8 sm:p-10">
        ...
        <form action={formAction} className="mt-8 space-y-5">
          <label className="block">
            ...
            <div className="relative">
              ...
              <input name="username" required autoComplete="username" defaultValue={devHint?.username} className="input-luxe pl-11" placeholder="admin" />
            </div>
          </label>
          <label className="block">
            ...
            <div className="relative">
              ...
              <input name="password" type="password" required autoComplete="current-password" defaultValue={devHint?.password} className="input-luxe pl-11" placeholder="••••••••" />
            </div>
          </label>
        ...
        {devHint && (
          <div className="mt-6 rounded-xl border border-gold/20 bg-gold/5 px-4 py-3 text-xs text-mist">
            <p className="flex items-center gap-1.5 font-semibold text-gold">
              <Info className="h-3.5 w-3.5" /> Development only
            </p>
            <p className="mt-1">
              Username: <span className="text-cream">{devHint.username}</span> · Password:{" "}
              <span className="text-cream">{devHint.password}</span>
            </p>
            <p className="mt-1 text-[11px] text-ash">This hint is hidden automatically in production.</p>
          </div>
        )}
